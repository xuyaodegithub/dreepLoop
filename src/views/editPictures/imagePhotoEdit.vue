    ...
    <div class="editPictures flex">
        ...
        <header class="header flex j-b a-i">
            ...
            <div class="hDown">
                ...
                <!--                    </div>-->
                <el-button class="doo" type="primary" icon="el-icon-download" slot="reference"
                           @click="downLoadImg($event,0)">下载
                ...
                <div class="flex smallBtn f-w">
                    <div v-for="(items,idx) in initcolorList" :key="idx">
                        <div
                                ...
                                class="cu"
                                :style="items | colorsStyle">
                            <i class="el-icon-check" v-show="colorType===items.val"></i>
                        </div>
                        ...
                        <p style="font-size: 12px;color:#fff;margin:8px 0 8px;text-align: center">{{items.name}}</p>
                    </div>
                    ...
                    <div class="iconList flex contents f-w j-b">
                        <div class="cu" v-for="(item,idx) in effectList" :key="idx" @click="loadThis(item,idx)">
                            <div class="its" :style="item | itemBack">
                                <img :src="item.imgurl"
                                     alt="">
                                <img class="upimgsb" v-if="item.type===2 && item.imgSrc.split(',').length>1"
                                     :src="item.imgSrc.split(',')[1]" alt="">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="e_r" id="e_r" @mouseup="blurAll">
            ...
            <div class="subs otherSubs"
                ...
                <div class="items upss" :class="{'hovers' : item.hovering,'teimgs' : item.type===3}"
                     v-for="(item,idx) in parseSubs.subList"
                     :style="item | subsStyle"
                     @mousedown.stop="moveBack($event,idx)" @click.stop="hoverThis(idx)" @mouseenter="borderFun(idx)"
                     @mouseleave="borderFun(idx,1)">
                    ...
                    <el-tooltip class="item" effect="dark" content="双击替换图片" placement="top"
                                :disabled="moveNum>0">
                        <div v-if="[1].includes(item.type)" class="Imgs" @dblclick="writeText(idx)"
                             :style="miansubStyle">
        ...
        <div class="e_rLast">
            <v-mune ref="Munes" @mattingImgs="repireImg"
                    @effectsImg="effectsImg" @loading="loadings" @hoverMain="hoverMain"
                    :edrieImgInfo="edrieImgInfo"></v-mune>
            <!--            <h4>修复</h4>-->
...
<script>
    ...
    import fupa from '@/assets/image/zdy.jpg';
    import color from '@/assets/image/tm.jpg';
    ...
    export default {
        ...
        data() {
            return {
                ...
                loadSubing: 0,
                initcolorList: [
                    {name: '自定义', val: '#', back: fupa}, {name: '透明', val: '', back: color}, {name: '红', val: '#FF0000'},
                    {name: '蓝', val: '#438edb'}, {name: '白', val: '#fff'}, {name: '深蓝', val: '#2a385b'}, {name: '青蓝', val: '#00BFF3'},
                    ...
                    {name: '希望绿', val: '#40896E'}, {name: '绅士红', val: '#C42134'},
        ...
        filters: {
            colorsStyle(item) {
                const a = item.val.split( ',' )
                return item.back ? {backgroundImage: `url(${item.back})`} : {background: `linear-gradient(${a[0]},${a[1] || a[0]})`}
            },
            subsStyle(item) {
                let data = {
        ...
        methods: {
            ...
            selectColor(item, idx) {
                ...
                if (idx > 1) this.parseSubs.subList[ix].backColor = this.colorType;
            ...
            choseColor(color) {//选择颜色背景，颜色选择器
                ...
                this.initcolorList[0].val = color;
            ...
            mattingAfter(result, type) {
                ...
                if (this.parseSubs.subList[idx].type === 1) {
                    ...
                    oImg.onload = () => {
                        ...
                        let downTop = h * 0.08, ih = (h - downTop) * oh / point.height, iw = ow * ih / oh,
                            left = w / 2 - iw / 2;//
                        let afterPoint = {
                            x: iw * point.x / ow,
                            y: ih * point.y / oh,
            ...
            initDiv() {
                let oImg = new Image(), w = 900, h = 1275, mattingMsg = {
                    "previewHeight": "500",
                    "original": "http://deeplor.oss-cn-hangzhou.aliyuncs.com/upload/image/20200912/68c0e3b336c74460958010c081d06529.jpg",
                    "originalHeight": "1600",
                    "maskRect": {"x": 427, "width": 896, "y": 95, "height": 1505},
                    "bgRemovedPreview": "https://deeplor.oss-cn-hangzhou.aliyuncs.com/matting2/2020/09/21/http___deeplor.oss-cn-hangzhou.aliyuncs.com_upload_image_20200912_68c0e3b336c74460958010c081d06529.png",
                    "queueNumber": "1",
                    "originalWidth": "1600",
                    "status": "success",
                    "fileId": "746894",
            ...
            loadThis(item, x) {
                let type = item.type, oCan = document.createElement( 'canvas' ), canTxt, data,
                    idx = this.parseSubs.subList.findIndex( items => !items.type ), mattingMsg = this.edrieImgInfo,
                    w = mattingMsg.w, h = mattingMsg.h;
                ...
                this.parseSubs.subList = [{
                    ...
                    pro: '',
                    ori: ''
                }, ...this.parseSubs.subList.filter( itemson => [1].includes( itemson.type ) )];
                if (type === 1) {
                    let gradient = canTxt.createRadialGradient( w / 2, h / 2, 0, w / 2, h / 2, h / 2 ),
                    ...
                    data = {
                        ...
                        type: 0,
                        title: '背景组件',
                        w: w * this.parseSubs.scale,
                        h: h * this.parseSubs.scale,
                        x: 0,
                        y: 0,
                        id: `img${Math.random()}`,
                        rotate: 0,
                        useImg: oCan.toDataURL(),
                        pro: oCan.toDataURL(),
                        ori: oCan.toDataURL(),
                        hovering: false,
                        mattingType: -1,
